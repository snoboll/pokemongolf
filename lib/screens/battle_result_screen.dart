...
class _EvolveRewardSectionState extends State<_EvolveRewardSection> {
  ...
  void didChangeDependencies() {
    ...
    // Award the Evo-HotDog exactly once. The battle's evolutionClaimed flag
    // guards against re-awarding when the result screen is revisited.
    if (!_awarded && !widget.battle.evolutionClaimed) {
      _awarded = true;
      BogeybeastGolfScope.of(context).addItem(ItemType.evoHotDog);
      BattleScope.of(context).claimEvolutionReward(widget.battle.id);
    }
  }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(ItemType.evoHotDog.icon, color: color, size: 26),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(
                  'Use it from Profile → Items to evolve a Bogeybeast.',
                  style: theme.textTheme.bodySmall?.copyWith(
